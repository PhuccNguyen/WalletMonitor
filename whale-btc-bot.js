...
async function getCurrentBitcoinPrice() {
  try {
    ...
    try {
      ...
      console.error("⚠️ Binance API also failed!", fallbackError);
      return "N/A";
    }
  }
}
...
// 📊 **Track BTC Price at a Specific Time**
async function getPriceAtTime(timestamp) {
  if (priceHistory.length === 0) return "N/A";
  let closest = priceHistory.reduce((prev, curr) =>
    Math.abs(curr.timestamp - timestamp) < Math.abs(prev.timestamp - timestamp) ? curr : prev
  );
  return closest.price || "N/A";
}
...
async function processTransaction(transaction) {
  ...
  inputs.forEach(input => {
    if (WHALE_WALLETS.has(input.prev_out.addr)) {
      ...
      totalSent += input.prev_out.value / 1e8; // Convert Satoshi to BTC
    }
  });
...
async function notifyWhaleTransaction(wallets, sent, received, txHash, btcPriceAtTime) {
  ...
  const priceAtTransaction = btcPriceAtTime !== "N/A" ? btcPriceAtTime : currentPrice;

  let message = `
🚨 <b>Whale Alert!</b> 🚨
💰 <b>Wallets Involved:</b> ${wallets.join(", ")}
🔄 <b>Transaction Details:</b>
${sent > 0 ? `📉 <b>Sent:</b> ${sent.toLocaleString()} BTC (${categorizeTransaction(sent)})` : ""}
${received > 0 ? `📈 <b>Received:</b> ${received.toLocaleString()} BTC (${categorizeTransaction(received)})` : ""}
📊 <b>BTC Price at Transaction:</b> $${priceAtTransaction}
...
📈 <b>Predicted Trend:</b> ${predictTrend(priceChange)}
📡 <i>Data from CoinGecko & Binance</i>
`;
...
function analyzeMarket(priceChange) {
  if (priceChange > 2) return "🔥 Strong Bullish Trend!";
  if (priceChange < -2) return "⚠️ Possible Bearish Reversal!";
  return "🔄 Neutral Trend.";
}

// 🔮 **Predict Next Move**
function predictTrend(priceChange) {
  if (priceChange > 2) return "🚀 Potential Breakout!";
  if (priceChange < -2) return "📉 Possible Correction Ahead!";
  return "📊 Market Waiting for Signal.";
}
